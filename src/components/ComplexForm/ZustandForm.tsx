...
import { useCellStore } from '../../store/zustandComplexStore'

const ZustandForm: React.FC = () => {
  const selectedCellIds = useCellStore(state => state.selectedCellIds)
  const cells = useCellStore(state => state.cells)
  const loading = useCellStore(state => state.loading)
  const selectCells = useCellStore(state => state.selectCells)
  const batchIncrement = useCellStore(state => state.batchIncrement)
  ...
  return (
    <div className="card">
      <h2 className="title">Zustand 复杂表单</h2>
